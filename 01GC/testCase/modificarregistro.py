...
class modificarregistro:
    ...
    def modificarregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            Log().error("No se pudo encontrar el campo para modificar el codigo alternativo revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        Registrosubgrupos = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='100']")))

        try:
            action = ActionChains(self.driver)
            action \
                .move_to_element(Registrosubgrupos) \
                .pause(0) \
                .double_click(Registrosubgrupos) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Se da clic en el registro creado, para proceder a modificarlo.")
        ...
        try:
            ...
            Ccodigoaltersub.clear()
            Ccodigoaltersub.send_keys(Configuracion.Mcodigoaltersub)
        ...
        try:
            ...
            Cdescripcionsub.clear()
            Cdescripcionsub.send_keys(Configuracion.Mdescripcionsub)
        ...
        Registroctascont = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='prueba']")))

        try:
            action = ActionChains(self.driver)
            action \
                .move_to_element(Registroctascont) \
                .pause(0) \
                .double_click(Registroctascont) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Se da clic en el registro creado, para proceder a modificarlo.")
