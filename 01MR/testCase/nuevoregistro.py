...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("MOTIVOS DE RECHAZO : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Trechazo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_trechazo))).text
            self.assertEqual("Tipo Rechazo", Trechazo, "Campo visible")
            Log().info(" El campo 'Tipo Rechazo' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Tipo Rechazo' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            CTrechazo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_trechazo)))
            CTrechazo.click()
            
            registro_trechazo = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Indistinto']")))

            action = ActionChains(self.driver)
            action \
                .click(registro_trechazo) \
                .pause(0) \
                .release()
            action.perform()
            ...
            Log().error(
                "No se encontró el registro de Tipo Rechazo, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
