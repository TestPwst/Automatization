...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("MOTIVOS DE DEVOLUCIÓN : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            ...
            Log().info(" El campo 'Deposito' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Deposito' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            DepositoDestino = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_deposito_destino))).text
            self.assertEqual("Depósito destino", DepositoDestino, "Campo visible")
            Log().info(" El campo 'Deposito Destino' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Deposito Destino' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Cantdias = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_cantidaddias))).text
            self.assertEqual("Cantidad de días", Cantdias, "Campo visible")
            Log().info(" El campo 'Cantidad de días' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Cantidad de días' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            registro_deposito = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='4151']")))
        ...
        try:
            ...
            registro_depositodestino = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='4154']")))
        ...
        try:
            ...
            Ccantdias.send_keys(Configuracion.Icantidaddias)
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
