...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            TiposEjecucionEEV = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='tipoejecucioneev']")))

            action = ActionChains(self.driver)
            action \
                .click(TiposEjecucionEEV) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Tipos de Ejecuciones EEV")
        ...
        try:
            ...
            self.assertEqual("TIPO DE EJECUCIÓN EEV", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Tipos de Ejecución EEV.")
            time.sleep(1)
