...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='mabcecgut']")))
            time.sleep(1)
            Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='mabcecgut']")))
            ...
            Log().error("No se encontró el registro creado, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
