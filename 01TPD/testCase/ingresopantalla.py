...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Distribuidor = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_cli_distrib']")))

            action = ActionChains(self.driver)
            action \
                .move_to_element(Distribuidor) \
                .pause(0) \
                .release()
            action.perform()

            Tiposdistribuidor = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='tiposdistribuidor']")))

            action = ActionChains(self.driver)
            action \
                .click(Tiposdistribuidor) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de tipos de distribuidor")
        ...
        try:
            ...
            self.assertEqual("TIPO DE DISTRIBUIDOR", Pantalla, "La pantalla ejecutada es correcta")
