...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro2 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='test']")))
            time.sleep(1)
            Registro2 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='test']")))
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Tipos Distribuidor.")
            time.sleep(2)
