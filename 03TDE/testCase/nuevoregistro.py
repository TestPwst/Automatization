...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("TIPOS ENCUESTAS : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Ctipo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_tipo)))
            Ctipo.click()

            registro_tipo = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Verificación(Finaliza encuesta)']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Se dió click en la opción Verificación(Finaliza encuesta).")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            ...
            Log().error("No se dió click en la opción Verificación(Finaliza encuesta), validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Nuevarespuesta = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_respuesta)))
            Nuevarespuesta.click()
            Log().info(" Se dió click en el botón Nuevo; se debe crear un nuevo registro de Respuestas.")

        except Exception as e:
            Log().error("No se dió click en el boton Nuevo de la pestaña respuestas, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Ccodigores = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_codigores)))
            Ccodigores.send_keys(Configuracion.Icodigores)
            Log().info(" Ingresa el Codigo del nuevo registro ")
        ...
        try:
            ...
            Cdescripcionres.send_keys(Configuracion.Idescripcionres)
            Log().info(" Ingresa la Descripción del nuevo registro ")

        except Exception as e:
            Log().error("No se pudo encontrar el campo para ingresar la Descripción, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Cfinalizaencuesta = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_finencuesta)))
            Cfinalizaencuesta.click()
            Log().info(" Se dió click en el checkbox Finaliza Encuesta.")

        except Exception as e:
            Log().error("No se dió click en el checkbox Finaliza Encuesta, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            Log().info(" Se da clic en el boton Guardar de la pestaña programas; se debe crear un nuevo registro.")

        except Exception as e:
            Log().error("No se dió click en el botón Guardar de la pestaña programas, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Ccodigores = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_codigores)))
            Ccodigores.send_keys(Configuracion.Icodigores2)
            Log().info(" Ingresa el Codigo del nuevo registro ")
        ...
        try:
            Corden = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_orden)))
            Corden.send_keys(Configuracion.Iorden2)
            Log().info(" Ingresa la Orden del nuevo registro ")
        ...
        try:
            Cdescripcionres = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_descripcionres)))
            Cdescripcionres.send_keys(Configuracion.Idescripcionres2)
            Log().info(" Ingresa la Descripción del nuevo registro ")
        ...
        try:
            Ccodigores = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_codigores)))
            Ccodigores.send_keys(Configuracion.Icodigores3)
            Log().info(" Ingresa el Codigo del nuevo registro ")
        ...
        try:
            Corden = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_orden)))
            Corden.send_keys(Configuracion.Iorden3)
            Log().info(" Ingresa la Orden del nuevo registro ")
        ...
        try:
            Cdescripcionres = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_descripcionres)))
            Cdescripcionres.send_keys(Configuracion.Idescripcionres3)
            Log().info(" Ingresa la Descripción del nuevo registro ")
