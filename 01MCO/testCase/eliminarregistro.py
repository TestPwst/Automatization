...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro = self.driver.find_element(By.XPATH, "//span[text()='"+Configuracion.Icodigo+"']")
            time.sleep(1)
            Registro = self.driver.find_element(By.XPATH, "//span[text()='"+Configuracion.Icodigo+"']")
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Modelos Cobranza.")
            time.sleep(2)
