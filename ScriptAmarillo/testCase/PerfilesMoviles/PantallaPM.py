...
class PantallaPM:
    ...
    def PantallaPM(self):
        ...
        Perfilesmoviles = self.driver.find_element(By.XPATH, "//*[@id='perfilesmoviles']")
        ...
        try:
            ...
            action \
                ...
                .pause(4) \
                .click(Perfilesmoviles) \
                .release()
            action.perform()
            time.sleep(5)
            Log().info(" Abre la pantalla de Perfiles Moviles")

        except Exception as e:
            Log().error("No se pudo encontrar el menu indicado, validar que la acción anterior haya finalizado, que el nombre de la pantalla sea el correcto o que la página no presente lentitud")
            time.sleep(3)
        ...
        try:
            ...
            self.assertEqual("PERFILES MÓVILES (PERFIL VENDEDOR)", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Perfiles Moviles")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
