...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("Tipo de Documento", Tipodocumento, "Campo visible")
            Log().info(" El campo 'Tipo de Documento' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Tipo de Documento' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Ctipodocumento.send_keys(Configuracion.Itipodoc)
            Log().info(" Ingresa el tipo de documento del nuevo registro ")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Cmoneda = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_moneda)))
            Cmoneda.send_keys("1")

        except Exception as e:
            Log().error("No se encontró la ayuda de Moneda, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Cantidadmixideal = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_cantmixideal))).text
            self.assertEqual("Cantidad Mix Ideal(Packing)", Cantidadmixideal, "Campo visible")
            Log().info(" El campo 'Cantidad Mix Ideal(Packing)' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Cantidad Mix Ideal(Packing)' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Ccantidadmixideal.send_keys(Configuracion.Icantmixideal)
            Log().info(" Ingresa la Cantidad de Mix Ideal del nuevo registro ")

        except Exception as e:
            Log().error("No se pudo encontrar el campo para ingresar la Cantidad de Mix Ideal, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Log().info(" Se presiona el boton 'Guardar', para guardar el primer registro de Por Articulo.")
        ...
        try:
            Nuevoporarticulo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_porarticulo)))
            Nuevoporarticulo.click()
            Log().info(" Se presiona el boton 'Nuevo', para crear el segundo registro de Por Articulo.")
        ...
        try:
            Ccantidadmixideal = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_cantmixideal)))
            Ccantidadmixideal.send_keys(Configuracion.Icantmixideal2)
            Log().info(" Ingresa la Cantidad de Mix Ideal del nuevo registro ")
        ...
        try:
            Guardarporarticulo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_porarticulo)))
            Guardarporarticulo.click()
            Log().info(" Se presiona el boton 'Guardar', para guardar el segundo registro de Por Articulo.")
        ...
        # Ingresa 3er articulo

        try:
            Nuevoporarticulo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_porarticulo)))
            Nuevoporarticulo.click()
            Log().info(" Se presiona el boton 'Nuevo', para crear el tercer registro de Por Articulo.")
        ...
        try:
            Ccantidadmixideal = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_cantmixideal)))
            Ccantidadmixideal.send_keys(Configuracion.Icantmixideal3)
            Log().info(" Ingresa la Cantidad de Mix Ideal del nuevo registro ")
        ...
        try:
            Guardarporarticulo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_porarticulo)))
            Guardarporarticulo.click()
            Log().info(" Se presiona el boton 'Guardar', para guardar el tercer registro de Por Articulo.")
