...
class modificarregistro:
    ...
    def modificarregistro(self):
        ...
        try:
            ...
            Chorainicio.clear()
            Chorainicio.send_keys(Configuracion.Mhorainicio)
            Log().info(" Se modifica el contenido del campo Hora Inicio ")
            time.sleep(3)

        except Exception as e:
            Log().error("No se pudo encontrar el campo para modificar la Hora inicio, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            Log().error("No se pudo encontrar el campo para modificar el número de orden, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            registro_sincro = self.driver.find_element(By.XPATH, "/html/body/div[3]/div[2]/ui-container/div[2]/ui-window/div[10]/div[2]/ui-container/ui-row[2]/ui-listview/div/div/div[2]/div/div")
        ...
        try:
            ...
            Cusuario.clear()
            Cusuario.send_keys(Configuracion.Musuario)
            Log().info(" Se modifica el contenido del campo usuario ")
            time.sleep(3)

        except Exception as e:
            Log().error("No se pudo encontrar el campo para modificar el Usuario, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            Ccontrasena.clear()
            Ccontrasena.send_keys(Configuracion.Mcontrasena)
            Log().info(" Se modifica el contenido del campo Contraseña ")
            time.sleep(3)

        except Exception as e:
            Log().error("No se pudo encontrar el campo para modificar la Contraseña, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
