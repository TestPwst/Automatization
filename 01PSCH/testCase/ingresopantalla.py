...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Programas = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='programassch']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Programas")
        ...
        try:
            ...
            self.assertEqual("PROGRAMAS", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Programas.")
            time.sleep(1)
            ...
            Log().error("La pantalla ejecutada no es correcta, validar que la acción anterior haya finalizado, que el mombe de la pantalla sea el correcto o que la página no presente lentitud")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
