...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            Log().error("No se dio click en el botón Refrescar, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(1)
        ...
        try:
            Formato = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_formato))).text
            self.assertEqual("Formato", Formato, "Campo visible")
            Log().info(" El campo 'Formato' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Formato' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            registro_ejecutar = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Reporte']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Se dió click en la opción Reporte.")
            
        except Exception as e:
            Log().error("No se dió click en la opción Reporte, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        # Validar etiqueta Cambiable

        try:
            Nombresalida = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_nombresalida))).text
            self.assertEqual("Nombre Salida", Nombresalida, "Campo visible")
            Log().info(" El campo 'Nombre Salida' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Nombre Salida' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Cformato = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_formato)))
            Cformato.click()
            
            registro_formato = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='HTML']")))

            action = ActionChains(self.driver)
            action \
                .click(registro_formato) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Se dió click en la opción HTML.")
            
        except Exception as e:
            Log().error("No se dió click en la opción HTML, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
