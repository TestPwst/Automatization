...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            Log().error("No se dió click en la Multiple Opción, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            Log().info(" Se dió click en el botón Nuevo; Para crae un nuevo registro de Respuesta.")
        ...
        try:
            ...
            Log().info(" Se da clic en el boton Guardar; se debe crear un nuevo registro de Respuesta.")
        ...
        try:
            ...
            Log().error("No se encontró el boton Nuevo de la pestaña respuestas, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Guardarespuesta = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_respuesta)))
            Guardarespuesta.click()
            Log().info(" Se da clic en el boton Guardar; se debe crear un nuevo registro de Respuestas.")
