...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            registro_buscador = self.wait.until(conditions.visibility((By.XPATH, "//div[text()='Motivos de no Fotos']")))
        ...
        try:
            ...
            self.assertEqual("MOTIVOS DE NO FOTOS", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Motivos de No Fotos.")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
