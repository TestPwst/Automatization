    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            action \
                ...
                .release()
            action.perform()
            Bancos = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_bancos']")))
            action = ActionChains(self.driver)
            action \
                .move_to_element(Bancos) \
                .pause(0) \
                .move_to_element(Bancos) \
                .pause(0) \
                .release()
            action.perform()
            ZonasBancarias = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='zonasbancarias']")))
            action = ActionChains(self.driver)
            action \
                .click(ZonasBancarias) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Zonas Bancarias")
        ...
        try:
            ...
            self.assertEqual("ZONAS BANCARIAS", Pantalla, "La pantalla ejecutada es correcta")
