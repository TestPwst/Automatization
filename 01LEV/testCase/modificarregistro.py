...
class modificarregistro:
    ...
    def modificarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='"+Configuracion.Icodigolista+"']")))
            time.sleep(2)
            Registro = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='"+Configuracion.Icodigolista+"']")))
        ...
        try:
            ...
            action \
                ...
                .pause(0) \
                .double_click(Registropreguntas) \
                .pause(0) \
                .release()
            action.perform()
            ...
            Log().error(
                "No se encontró el registro de preguntas, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Cpregunta = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_pregunta)))
            Cpregunta.clear()
            Cpregunta.send_keys(Configuracion.Mpregunta)
            Log().info(" Ingresa la pregunta del nuevo registro ")

        except Exception as e:
            Log().error(
                "No se pudo encontrar el campo para ingresar la pregunta, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Guardapreguntas = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_pregunta)))
            Guardapreguntas.click()
