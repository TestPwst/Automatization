...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Promociones = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_promo']")))

            action = ActionChains(self.driver)
            action \
                .move_to_element(Promociones) \
                .pause(0) \
                .release()
            action.perform()

            ListasEvaluacion = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='listaspromoevaluaciones']")))

            action = ActionChains(self.driver)
            action \
                .click(ListasEvaluacion) \
                .release()
            action.perform()

            Log().info(" Abre la pantalla de Listas Evaluación")
        ...
        try:
            ...
            self.assertEqual("LISTAS DE EVALUACIÓN", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Listas Evaluación.")
        ...
        try:
            ...
            Registrorepetido = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='"+Configuracion.Icodigolista+"']")))
            self.wait = WebDriverWait(self.driver, 60)
            ...
            if Registrorepetido.is_displayed():
                ...
                # Cambio Pestaña subclases

                try:
                    Bpreguntas = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_preguntas)))
                    Bpreguntas.click()
                    Log().info("Se hace el cambio de pestaña para continuar con el registro nuevo")
                    
                except Exception as e:
                    Log().error(
                        "No se encontró el botón preguntas, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                    self.driver.get_screenshot_as_file(img_name)
                ...
                try:
                    ...
                    Registropreguntas = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Codigopreg']")))

                    action = ActionChains(self.driver)
                    action \
                        .move_to_element(Registropreguntas) \
                        .pause(0) \
                        .click(Registropreguntas) \
                        .pause(0) \
                        .release()
                    action.perform()
                    self.wait = WebDriverWait(self.driver, 60)
                ...
                try:
                    Eliminapreguntas = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_preguntas)))
                    Eliminapreguntas.click()
                ...
                try:
                    Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='"+Configuracion.Icodigolista+"']")))
