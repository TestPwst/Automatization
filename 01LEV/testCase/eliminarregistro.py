...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro2 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='"+Configuracion.Icodigolista+"']")))
            time.sleep(1)
            Registro2 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='"+Configuracion.Icodigolista+"']")))
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Listas Evaluación.")
            time.sleep(2)
