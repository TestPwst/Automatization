...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            time.sleep(3)
            Nuevo2 = self.wait.until(conditions.clickable((By.XPATH, Configuracion.btn_Nuevo)))
            Nuevo2.click()
        ...
        try:
            ...
            Log().error("El campo 'Frecuencia' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
