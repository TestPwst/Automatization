...
class modificarregistro:
    ...
    def modificarregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            registro_tipomoneda = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Extranjera']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Se dió click en la opción Extranjera.")

        except Exception as e:
            Log().error("No se dió click en la opción Extranjera, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            registro_perdida = self.wait.until(conditions.visibility((By.XPATH, "/html/body/div[3]/div[2]/ui-container/div/ui-window/div[10]/div[2]/ui-container/ui-row[2]/ui-listview/div/div/div[2]/div/div[2]/div[1]")))
        ...
        try:
            ...
            registro_ganancia = self.wait.until(conditions.visibility((By.XPATH, "/html/body/div[3]/div[2]/ui-container/div/ui-window/div[10]/div[2]/ui-container/ui-row[2]/ui-listview/div/div/div[2]/div/div[2]")))
        ...
        try:
            ...
            registro_clasificacion = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Prueba Cambio']")))
        ...
        # Cambio Pestaña observaciones

        try:
            Bobservaciones = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_observaciones)))
            Bobservaciones.click()
            Log().info("Se hace el cambio a la pestaña Observaciones para continuar con la modificación del registro ")
        ...
        try:
            ...
            CObservaciones1.clear()
            CObservaciones1.send_keys(Configuracion.Mobservacion1)
            Log().info(" Se modifica el contenido del campo Observaciones 1 ")

        except Exception as e:
            Log().error("No se pudo encontrar el campo para Modificar la observación 1, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            CObservaciones2.clear()
            CObservaciones2.send_keys(Configuracion.Mobservacion2)
            Log().info(" Se modifica el contenido del campo Observaciones 2 ")

        except Exception as e:
            Log().error("No se pudo encontrar el campo para modificar la observación 2, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            CObservaciones3.clear()
            CObservaciones3.send_keys(Configuracion.Mobservacion3)
            Log().info(" Se modifica el contenido del campo Observaciones 3 ")

        except Exception as e:
            Log().error("No se pudo encontrar el campo para modificar la observación 3, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
