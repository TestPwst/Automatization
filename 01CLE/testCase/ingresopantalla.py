...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            ClaseEquipos = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='clasesE']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Clases Equipos")
        ...
        try:
            ...
            self.assertEqual("CLASES DE EQUIPOS", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Clases Equipos.")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='CodigoTest']")
            if Registrorepetido.is_displayed():
                ...
                Registrosubclases = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='CodigoT001']")))

                try:
                    action = ActionChains(self.driver)
                    action \
                        .move_to_element(Registrosubclases) \
                        .pause(0) \
                        .click(Registrosubclases) \
                        .pause(0) \
                        .release()
                    action.perform()
                    Log().info(" Se da clic en el registro creado, para proceder a eliminarlo.")
                    ...
                    Log().error("No se encontró el registro de subclases, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
                    time.sleep(3)
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                ...
                try:
                    Eliminasubclases = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_subclases)))
                    Eliminasubclases.click()
                ...
                Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='CodigoTest']")))
