...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro2 = self.wait.until(conditions.visibility((By.XPATH, "//span[contains(text(), '( 3)')]")))
            time.sleep(1)
            Registro2 = self.wait.until(conditions.visibility((By.XPATH, "//span[contains(text(), '( 3)')]")))
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Modelos Liquidación.")
            time.sleep(2)
