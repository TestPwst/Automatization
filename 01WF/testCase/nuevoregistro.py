...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            Btiposdoc.click()
            Log().info("Se hace el cambio de pestaña Tipos Documento para continuar con el registro nuevo")
        ...
        try:
            Nuevo2 = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_tipodoc)))
            Nuevo2.click()
            Log().info(" Se presiona el boton 'Nuevo', para crear un nuevo registro de Tipos Documento.")
        ...
        try:
            Modbloqueo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_modobloqueo))).text
            self.assertEqual("Modo Bloqueo", Modbloqueo, "Campo visible")
            Log().info(" El campo 'Modo Bloqueo' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Modo Bloqueo' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Obligatorio = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_obligatorio))).text
            self.assertEqual("Obligatorio", Obligatorio, "Campo visible")
            Log().info(" El campo 'Obligatorio' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Obligatorio' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            registro_modbloqueo = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Bloquea']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Se dió click en el regitro de Modo Bloqueo.")

        except Exception as e:
            Log().error("No se encontró la opción de Modo Bloqueo, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            GuardarTipoDoc = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_tipodoc)))
            GuardarTipoDoc.click()
            Log().info(" Se presiona el boton 'Guardar', para guardar el registro Tipos Documento.")
