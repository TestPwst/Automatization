...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            WorksFlow = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='workflowsperfil']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Worksflow")
        ...
        try:
            ...
            self.assertEqual("WORK FLOWS (PERFIL MÓVIL)", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Work Flows")
            time.sleep(3)
            ...
            Log().error("La pantalla ejecutada no es correcta, validar que la acción anterior haya finalizado, que el nombre de la  sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
