...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("MODELOS DE RESPUESTA : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            ...
            Ccodigo.send_keys(Configuracion.Mcodigoalter)
        ...
        try:
            Nuevorespuesta = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_respuestas)))
            Nuevorespuesta.click()
        ...
        try:
            rcodigo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_rcodigo))).text
            self.assertEqual("Código", rcodigo, "Campo visible")
            Log().info(" El campo 'Código base' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Código' no se muestra visible, revise si el xpath sigue siendo el mismo, revise la accion anterior si el xpath es el correcto o el tiempo de la accion")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            crcodigo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_rcodigo)))
            crcodigo.send_keys(Configuracion.Rcodigo)
        ...
        try:
            ...
            crorden.send_keys(Configuracion.Rorden)
            Log().info(" Ingresa la Clave Correlativo del nuevo registro ")
        ...
        try:
            ...
            crdescripcion.send_keys(Configuracion.Rdescripcion)
            Log().info(" Ingresa la Clave Correlativo del nuevo registro ")
        ...
        try:
            ...
            crpeso.send_keys(Configuracion.Rpeso)
            Log().info(" Ingresa la Clave Correlativo del nuevo registro ")
