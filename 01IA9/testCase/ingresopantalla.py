...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            InfArticulo9 = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='infoarticulo9']")))
            ...
            Log().info(" Abre la pantalla de Información del Artículo 9")
        ...
        try:
            ...
            self.assertEqual("INFORMACÍÓN DE ARTÍCULO9", Pantalla, "La pantalla ejecutada es correcta")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            Log().info(" La pantalla ejecutada es INFORMACIÓN DE ARTÍCULO 9.")
