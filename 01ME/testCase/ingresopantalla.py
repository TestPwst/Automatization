...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            MarcasEquipos = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='marcasE']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Marcas de Equipo")
        ...
        try:
            ...
            self.assertEqual("MARCAS DE EQUIPO", Pantalla, "La pantalla ejecutada es correcta")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            Log().info(" La pantalla ejecutada es Marcas de Equipos.")
        ...
        try:
            ...
            if Registrorepetido.is_displayed():
                ...
                try:
                    Bmodelos = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_configuracion_modelos)))
                    Bmodelos.click()
                    Log().info("Se hace el cambio de pestaña para continuar con el registro nuevo")
                    
                except Exception as e:
                    Log().error(
                        "No se encontró el botón modelos, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                    self.driver.get_screenshot_as_file(img_name)
                ...
                try:
                    Registro4 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Testing1']")))
                ...
                try:
                    Eliminamodelos = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_modelos)))
                    Eliminamodelos.click()
                    Log().info(" Se presiona el boton 'Eliminar Modelos', para eliminar el registro.")

                except Exception as e:
                    Log().error("No se encontró el botón Eliminar Modelos, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                    self.driver.get_screenshot_as_file(img_name)
                ...
                try:
                    Registro5 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='CódigoTest']")))
