...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("MOTIVOS DE NO ENCUESTADO : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Motivo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_motivo))).text
            self.assertEqual("Habilitar/deshabilitar motivo", Motivo, "Campo visible")
            Log().info(" El campo 'Motivo' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Motivo' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Cmotivo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_motivo)))
            Cmotivo.click()
            Log().info(" Hizo click en el checkbox ")
            
        except Exception as e:
            Log().error("No se pudo encontrar el campo motivo, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
