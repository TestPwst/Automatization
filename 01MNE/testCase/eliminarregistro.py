...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='CódigoTesting001']")))
            time.sleep(1)
            Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='CódigoTesting001']")))
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Motivos de No Encuestado.")
            time.sleep(2)
