...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            MotivosNoEn = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='motivosdenoencuestado']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Motivos de no encuestado")
        ...
        try:
            ...
            self.assertEqual("MOTIVOS DE NO ENCUESTADO", Pantalla, "La pantalla ejecutada es correcta")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            Log().info(" La pantalla ejecutada es Motivos de no encuestado.")
