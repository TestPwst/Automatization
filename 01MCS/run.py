...
discover = defaultTestLoader.discover(start_dir=case_path, pattern='motivosstock.py', top_level_dir=None)
...
if __name__ == "__main__":
    ...
    mycursor.execute("SELECT count(*) FROM guardar WHERE color='azul cielo' AND  ventana='01MCS'")
    myresult = mycursor.fetchall()
    value = myresult[-1][-1]
    "print(value)"
    if (value != 0):
        sql = "UPDATE guardar SET link = %s WHERE color=%s AND  ventana=%s"
        val = (new_path, "azul cielo", "01MCS")
