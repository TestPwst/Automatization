...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        # Cambio de pestaña niveles

        try:
            Bniveles = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_niveles)))
            Bniveles.click()
            Log().info("Se hace el cambio de pestaña para continuar con el registro nuevo")
            
        except Exception as e:
            Log().error("No se dió click al botón niveles, validar que la acción anterior haya finalizado,que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Registrodocumentos = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Tipos de cliente']")))
            ...
            Log().error(
                "No se encontró el registro, validar que la acción anterior haya finalizado,que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Eliminaniveles = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_niveles)))
            Eliminaniveles.click()
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Tipos presupuestos.")
            time.sleep(2)
