...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            Refresca = self.wait.until(conditions.clickable((By.XPATH, Configuracion.btn_Refresca)))
            Refresca.click()
        ...
        try:
            ...
            self.assertEqual("TIPOS DE PRESUPUESTOS : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            modopresupuesto = self.wait.until(conditions.visibility((By.XPATH, Configuracion. etiqueta_modopresupuesto))).text
            self.assertEqual("Modo presupuesto", modopresupuesto, "Campo visible")
            Log().info(" El campo 'Modo presupuesto' si se encuentra visible.")
        
        except Exception as e:
            Log().error(
                "El campo 'modo presupuesto' no se muestra visible, validar que la acción anterior haya finalizado,que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Cmodopresupuesto = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_modopresupuesto)))
            Cmodopresupuesto.click()
            
            registro_presupuesto = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Presupuesto por mes']")))

            action = ActionChains(self.driver)
            action \
                .click(registro_presupuesto) \
                .pause(0) \
                .release()
            action.perform()
            ...
            Log().error(
                "No se encontró el registro de modelo presupuesto, validar que la acción anterior haya finalizado,que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Log().error("No se dió click al botón Cerrar de la ventana., validar que la acción anterior haya finalizado,que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
