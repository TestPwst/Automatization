...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            registro_formulapedido = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Presupuesto por semana']")))
            ...
            Log().error(
                "No se encontró el registro modo presupuesto, validar que la acción anterior haya finalizado,que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Nuevoniveles = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_niveles)))
            Nuevoniveles.click()
        ...
        try:
            Descripcion = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_niveles_descripcion))).text
            self.assertEqual("Descripción", Descripcion, "Campo visible")
        ...
        try:
            categoria = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_categoria))).text
            self.assertEqual("Categoría", categoria, "Campo visible")
            Log().info(" El campo 'Categoría' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Categoría' no se muestra visible, validar que la acción anterior haya finalizado,que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Cdescripcionniveles = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_descripcionniveles)))
            Cdescripcionniveles.send_keys(Configuracion.Ndescripcion)
        ...
        try:
            Ccategoria = self.wait.until(conditions.visibility((By.XPATH, Configuracion.ayuda_categoria)))
            Ccategoria.click()
            
            registro_categoria = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='4']")))

            action = ActionChains(self.driver)
            action \
                .double_click(registro_categoria) \
                .pause(0) \
                .release()
            action.perform()

        except Exception as e:
            Log().error("No se encontró el registro de categoria, validar que la acción anterior haya finalizado,que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Guardarniveles = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_niveles)))
            Guardarniveles.click()
