...
class modificarregistro:
    ...
    def modificarregistro(self, conditions, Configuracion):
        ...
        Registro = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Codigopru1']")))
        ...
        try:
            ...
            registro_aplicablea = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Artículos']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Se dió click en la opción Articulos.")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            ...
            Log().error("No se dió click en la opción Articulos, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
