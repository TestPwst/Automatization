...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Modoscancelacion = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='modoscancelacion']")))

            action = ActionChains(self.driver)
            action \
                .click(Modoscancelacion) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de MODOS DE CANCELACION")
        ...
        try:
            ...
            self.assertEqual("MODOS DE CANCELACIÓN", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es MODOS DE CANCELACION")
