...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Depositos = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_dep']")))

            action = ActionChains(self.driver)
            action \
                .move_to_element(Depositos) \
                .pause(0) \
                .release()
            action.perform()

            ClasifDepositos = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='clasificacionesdeposito']")))

            action = ActionChains(self.driver)
            action \
                .click(ClasifDepositos) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Clasificación de Deposito")
        ...
        try:
            ...
            self.assertEqual("CLASIFICACIÓN DE DEPÓSITO", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Clasifiación de Deposito")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='CódigoTestingPrueba1']")
        ...
        try:
            ...
            Log().error("No se dio click el botón Nuevo, validar que la acción anterior haya finalizado,"
                        "que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
