...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='CódigoTestingPrueba1']")))
        ...
        try:
            ...
            Log().error("No se dio click al registro, validar que la acción anterior haya finalizado,"
                        "que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Clasificacion Depositos.")
            time.sleep(2)
