...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Prueba Cambio AutomatizadoM']")))
            time.sleep(1)
            Registro = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Prueba Cambio AutomatizadoM']")))
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Presupuestos.")
            time.sleep(2)
