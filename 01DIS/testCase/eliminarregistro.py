...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='1']")))
        ...
        Registrocuentas = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='prueba cambio']")))

        try:
            action = ActionChains(self.driver)
            action \
                .move_to_element(Registrocuentas) \
                .pause(0) \
                .click(Registrocuentas) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Se da clic en el registro creado, para proceder a modificarlo.")
        ...
        try:
            Eliminacuentas = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_CuentasCont)))
            Eliminacuentas.click()
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Distribuidores.")
            time.sleep(2)
