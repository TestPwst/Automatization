...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            Log().error("No se dio click al botón Refrescar, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
        ...
        try:
            ...
            self.assertEqual("INFORMACIÓN DE ARTÍCULO2 : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            ...
            Log().info(" Ingresa el codigo del nuevo registro igual al anterior")
        ...
        try:
            ...
            Log().info(" Ingresa la descripción del nuevo registro igual al anterior")
        ...
        try:
            ...
            Log().error("No se dio click al botón Cerrar de la ventana, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
