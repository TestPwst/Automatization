...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("PROGRAMACIONES : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Cdias = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_dias)))
            Cdias.click()

            registro_dias = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='TODOS LOS DIAS']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Se dió click en la opción TODOS LOS DIAS.")

        except Exception as e:
            Log().error("No se dió click en la opción Todos los Dias, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Choras = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_horas)))
            Choras.click()

            registro_horas = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='00:00 - Medianoche']")))

            action = ActionChains(self.driver)
            action \
                .click(registro_horas) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Se dió click en la opción 00:00 - Medianoche.")

        except Exception as e:
            Log().error("No se dió click en la opción 00:00 - Medianoche, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Cminutos = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_minutos)))
            Cminutos.click()

            registro_minutos = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='00 - Hora en punto']")))

            action = ActionChains(self.driver)
            action \
                .click(registro_minutos) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Se dió click en la opción 00 - Hora en punto.")

        except Exception as e:
            Log().error("No se dió click en la opción 00 - Hora en punto, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
