...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Dcheques = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='destinoscheques']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Destino Cheques")
        ...
        try:
            ...
            self.assertEqual("DESTINO CHEQUES", Pantalla, "La pantalla ejecutada es correcta")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            Log().info(" La pantalla ejecutada es Destino Cheques.")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='Testing001']")
            if Registrorepetido.is_displayed():
                ...
                try:
                    Registro4 = self.driver.find_element(By.XPATH, "/html/body/div[3]/div[2]/ui-container/ui-window[2]/div[10]/div[2]/ui-container/ui-row/ui-tabstrip/div/div[3]/ui-container/ui-row/ui-container/ui-row[2]/ui-listview/div/div/div[2]/div/div/div[1]")
                    if Registro4.is_displayed():
                        try:
                            action = ActionChains(self.driver)
                            action \
                        ...
                        try:
                            ...
                            Log().error(
                                "No se dio click al botón Eliminar destino, validar que la acción anterior haya finalizado,"
                                "que el xpath sea el correcto o que la página no presente lentitud")
                            time.sleep(2)
                            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
