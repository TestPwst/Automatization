...
class modificarregistro:
    ...
    def modificarregistro(self, conditions, Configuracion):
        ...
        Registro = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Testing001']")))
        ...
        try:
            ...
            registro_mliquidacion = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='No afecta liquidación vendedor']")))
        ...
        try:
            ...
            Log().error("No se pudo encontrar el campo adeudado, validar que la acción anterior haya finalizado,"
                        "que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
