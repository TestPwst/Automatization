...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            ClasArt = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='clasart']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de clasificación de artículos")
        ...
        try:
            ...
            self.assertEqual("CLASIFICACIÓN DE ARTÍCULO", Pantalla, "La pantalla ejecutada es correcta")
        ...
        try:
            ...
            Log().error("No se encontró el botón Nuevo, validar que la acción anterior haya finalizado,"
                        "que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
