...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Liquidaciones = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_liq']")))

            action = ActionChains(self.driver)
            action \
                .move_to_element(Liquidaciones) \
                .pause(0) \
                .release()
            action.perform()

            Tiposdistribucion = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='tiposdistribucion']")))

            action = ActionChains(self.driver)
            action \
                .click(Tiposdistribucion) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de tipos de Distribución")
        ...
        try:
            ...
            self.assertEqual("TIPOS DE DISTRIBUCIÓN", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es TIPOS DE PRESUPUESTOS")
