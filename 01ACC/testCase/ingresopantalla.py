...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        # Abre la pantalla
        try:
            CBuscador = self.wait.until(conditions.visibility((By.XPATH, Configuracion.buscador)))
            CBuscador.click()
            Log().info("Se click en la opcion del buscador")
            CBuscador2 = self.wait.until(conditions.visibility((By.XPATH, Configuracion.buscador2)))
            CBuscador2.send_keys(Configuracion.Iopcionbuscador)
            Log().info("Se escribe el nombre de la pantalla en el cuadro de texto")

            registro_buscador = self.wait.until(conditions.visibility((By.XPATH, "//div[text()='Acuerdos Comerciales']")))
            ...
            Log().error("No se pudo encontrar el campo del buscador, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            time.sleep(2)
        ...
        try:
            ...
            self.assertEqual("ACUERDOS COMERCIALES", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Acuardos Comerciales.")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            try:
                Refrescainicio = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Refresca)))
                Refrescainicio.click()
