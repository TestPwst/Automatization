...
class repetirregistro:
    ...
    def repetirregistro(self):
        ...
        try:
            Numcontrato = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_numcontrato))).text
            self.assertEqual("Nro. Contrato", Numcontrato, "Campo visible")
            Log().info(" El campo 'Codigo Numero Contrato' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Numero Contrato' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Fechafin = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_fecha_fin))).text
            self.assertEqual("Fecha Finalización", Fechafin, "Campo visible")
            Log().info(" El campo 'Fecha Finalización' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Fecha Finalización' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Log().error(
                "El campo 'Cuenta' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Quienauth = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_autoriza))).text
            self.assertEqual("Quien Autoriza", Quienauth, "Campo visible")
            Log().info(" El campo 'Quien Autoriza' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Quien Autoriza' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Log().info(" El campo 'Estado' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Estado' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Sucursal = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_sucursal))).text
            self.assertEqual("Sucursal", Sucursal, "Campo visible")
            Log().info(" El campo 'Sucursal' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Sucursal' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Log().error(
                "No se pudo encontrar el campo para ingresar el número de contrato, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            registro_cuenta = self.driver.find_element(By.XPATH, "//span[text()='0010447891']")
            ...
            Log().error(
                "No se encontró el registro de cuenta, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            registro_sucursal = self.driver.find_element(By.XPATH, "//span[text()='AV FUERZA AEREA S/N']")
