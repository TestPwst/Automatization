...
class modificarregistro:
    ...
    def modificarregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            Cnumcontrato.clear()
            Cnumcontrato.send_keys(Configuracion.Mcontrato)
        ...
        try:
            ...
            registro_cuenta = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='807H138843']")))
        ...
        try:
            ...
            registro_sucursal = self.wait.until(conditions.visibility((By.XPATH, "//span[text()= 'Blvd. Paseo ixtapa plaza ambiente lt 11']")))
        ...
        Registro2 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='11040']")))
        ...
        try:
            Eliminapolitica = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_politicas)))
            Eliminapolitica.click()
            Log().info(" Se presiona el boton 'Eliminar Politica', para eliminar el registro.")

        except Exception as e:
            Log().error("No se encontró el botón Eliminar Politica, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        # Modificación politicas

        try:
            Nuevopoliticam = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_politicas)))
            Nuevopoliticam.click()
        ...
        try:
            politicas = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_politicas))).text
            self.assertEqual("Políticas de Ventas:", politicas , "Campo visible")
            Log().info(" El campo 'Políticas de Ventas' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Politicas de Venta' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            registro_politica = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='840006']")))
