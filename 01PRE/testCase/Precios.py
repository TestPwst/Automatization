...
class Test(unittest.TestCase):
	...
	"""Escenario 1 de Clasificación Cuentas Contables"""
	@classmethod
	def test(self):
		...
		if (continua):
			...
			if (success == False):
				try:
					...
					Log().error(
						"No se encontró el botón Cerrar, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
					time.sleep(2)
			...
			Log().error(
				"La pantalla anterior fallo por lo que esta no funcionara, para mas detalles del error consulte el reporte")
			raise Exception()
