...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Precios = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='abmprecios']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Precios")
        ...
        try:
            ...
            self.assertEqual("PRECIOS", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Precios.")
        ...
        try:
            ...
            Log().error("No se encontrarón los filtros revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            filtro_articulo = self.driver.find_element(By.XPATH, Configuracion.filtro_articulo)
            action = ActionChains(self.driver)
            action \
                .move_to_element(filtro_articulo) \
                .pause(0) \
                .double_click(filtro_articulo) \
                .pause(0) \
                .release()
            action.perform()
            time.sleep(1)
            ...
            Log().error("No se encontrarón el filtro de articulos revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            filtro_listaprecio = self.driver.find_element(By.XPATH, Configuracion.filtro_listaprecio)

            action = ActionChains(self.driver)
            action \
                .move_to_element(filtro_listaprecio) \
                .pause(0) \
                .double_click(filtro_listaprecio) \
                .pause(0) \
                .release()
            action.perform()
            time.sleep(1)
            ...
            Log().error(
                "No se encontró el filtro de lista de precios revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        # Ingreso datos filtros

        try:
            Carticulofiltro = self.driver.find_element(By.XPATH, Configuracion.ayuda_articulofiltro)
            Carticulofiltro.click()
            time.sleep(1)

            registro_articulofiltro = self.driver.find_element(By.XPATH, "//span[text()='40041615']")

            action = ActionChains(self.driver)
            action \
                .double_click(registro_articulofiltro) \
                .pause(0) \
                .release()
            action.perform()
        ...
        try:
            Clistafiltro = self.driver.find_element(By.XPATH, Configuracion.ayuda_listapreciofiltro)
            Clistafiltro.click()
            time.sleep(1)

            registro_listafiltro = self.driver.find_element(By.XPATH, "//span[text()='Lista de Precios SCRP2.0 INTERIOR (16% IVA)']")

            action = ActionChains(self.driver)
            action \
                .double_click(registro_listafiltro) \
                .pause(0) \
                .release()
            action.perform()
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='MONARCA CAJITA DE BOLSILLO']")
