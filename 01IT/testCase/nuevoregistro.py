...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("INSTANTÁNEA DE TABLA : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Tablaorigen = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_tablaorigen))).text
            self.assertEqual("Tabla origen", Tablaorigen, "Campo visible")
            Log().info(" El campo 'Tabla origen' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Tabla origen' no se muestra visible, revise si el xpath sigue siendo el mismo, revise la accion anterior si el xpath es el correcto o el tiempo de la accion")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Ctablaorigen = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_tablaorigen)))
            Ctablaorigen.send_keys(Configuracion.Itablaorigen)
            Log().info(" Ingresa la Tabla Origen del nuevo registro ")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            ...
            Log().error("No se pudo encontrar el campo para ingresar la Tabla Origen revise si el xpath sigue siendo el mismo, revise la accion anterior si el xpath es el correcto o el tiempo de la accion")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
