...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro2 = self.driver.find_element(By.XPATH, "//span[text()='"+Configuracion.Icodigo+"']")
            time.sleep(1)
            Registro2 = self.driver.find_element(By.XPATH, "//span[text()='"+Configuracion.Icodigo+"']")
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Instancia Tabla.")
            time.sleep(2)
