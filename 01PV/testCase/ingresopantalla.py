...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Log().error("La pantalla ejecutada no es correcta, validar que la acción anterior haya finalizado, que el nombre de la pantalla sea el correcto o que la página no presente lentitude")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            if Registrorepetido.is_displayed():
                ...
                try:
                    ...
                    Bporarticulo.click()
                    Log().info("Se hace el cambio a la pestaña Por Articulo para continuar con la eliminación del registro")
                ...
                try:
                    ...
                    Registroporarticulo = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='MARLBORO LS BOX 14 (FA01003)']")))
                    self.wait = WebDriverWait(self.driver, 60)
                    ...
                    action \
                        ...
                        .release()
                    action.perform()
                    Log().info(" Se da clic en el registro Por Articulo, para proceder a eliminarlo.")
                    ...
                    Log().error("No se dió click en el registro de Por Articulo, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
                    time.sleep(1)
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                ...
                try:
                    ...
                    Log().info(" Se presiona el boton 'Eliminar', para eliminar el registro de Por Articulo.")
