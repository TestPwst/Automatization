...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Ubicacionesactivos = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='ubicacionesactivos']")))

            action = ActionChains(self.driver)
            action \
                .click(Ubicacionesactivos) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Ubicaciones Activos Fijos")
        ...
        try:
            ...
            self.assertEqual("UBICACIONES DE ACTIVOS FIJOS", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Ubicaciones Activos Fijos")
        ...
        try:
            ...
            Log().error(
                "No se dió click al botón Nuevo, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
