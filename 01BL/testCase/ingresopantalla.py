...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Estandares = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_estandares']")))

            action = ActionChains(self.driver)
            action \
                .move_to_element(Estandares) \
                .pause(0) \
                .release()
            action.perform()

            Bloques = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='bloqueseev']")))
        ...
        try:
            ...
            self.assertEqual("BLOQUES", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Bloques.")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='Testing01']")
            if Registrorepetido.is_displayed():
                ...
                # Cambio Pestaña Sub Bloques

                try:
                    Bsubbloques = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_subbloques)))
                    Bsubbloques.click()
                    Log().info("Se hace el cambio de pestaña para continuar con el registro nuevo")

                except Exception as e:
                    Log().error("No se encontró el botón subbloques, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
                    time.sleep(2)
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                ...
                Registrosubbloques = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Testing02']")))

                try:
                    action = ActionChains(self.driver)
                    action \
                        .move_to_element(Registrosubbloques) \
                        .pause(0) \
                        .double_click(Registrosubbloques) \
                        .pause(0) \
                        .release()
                    action.perform()
                ...
                try:
                    ...
                    Log().error("No se encontró el botón items, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
                    time.sleep(2)
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                ...
                Registroitems = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Testing03']")))
                ...
                try:
                    Eliminaitems = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_items)))
                    Eliminaitems.click()
                ...
                try:
                    Guardasubbloques = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_subbloques)))
                    Guardasubbloques.click()
                ...
                Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Testing01']")))
                ...
                try:
                    Eliminasubbloques = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_subbloques)))
                    Eliminasubbloques.click()
                    Log().info(" Se presiona el boton 'Eliminar', para eliminar el registro.")
                ...
                Registro4 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Testing01']")))
