...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            InfArticulo = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_art_info']")))

            action = ActionChains(self.driver)
            action \
                .move_to_element(InfArticulo) \
                .pause(0) \
                .release()
            action.perform()

            InfArticulo10 = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='infoarticulo10']")))
            ...
            Log().info(" Abre la pantalla de Información del Artículo 10")
        ...
        try:
            ...
            self.assertEqual("INFORMACÍÓN DE ARTÍCULO10", Pantalla, "La pantalla ejecutada es correcta")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            Log().info(" La pantalla ejecutada es INFORMACIÓN DE ARTÍCULO 10.")
