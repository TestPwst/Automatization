...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            Vencimiento = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_vencimiento))).text
            self.assertEqual("Modo control vencimiento", Vencimiento, "Campo visible")
            Log().info(" El campo 'Vencimiento' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Vencimiento' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Adeudos = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_adeudos))).text
            self.assertEqual("Modo control cheques adeudados", Adeudos, "Campo visible")
            Log().info(" El campo 'Adeudos' si se encuentra visible.")
        ...
        try:
            MedioPago = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_mediopago))).text
            self.assertEqual("Único medio de pago en el documento", MedioPago, "Campo visible")
            Log().info(" El campo 'Medio de Pago' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Medio de Pago' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            GenDoc = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_gendoc))).text
            self.assertEqual("Genera documento", GenDoc, "Campo visible")
            Log().info(" El campo 'Genera documento' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Genera documento' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Log().error("El campo 'Tipo documento cheque' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            VerficaCredito = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_verificacredito))).text
            self.assertEqual("Verificar vencimiento por límite de crédito", VerficaCredito, "Campo visible")
            Log().info(" El campo 'Verifica crédito' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Verifica crédito' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Liberar = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_liberar))).text
            self.assertEqual("Liberar crédito", Liberar, "Campo visible")
            Log().info(" El campo 'Liberar crédito' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Liberar crédito' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
