...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("GRUPO DE POLÍTICAS DE VENTA : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            TipoGrupo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_tipogrupo))).text
            self.assertEqual("Tipo de grupo", TipoGrupo, "Campo visible")
            Log().info(" El campo 'Tipo de grupo' si se encuentra visible.")
        ...
        try:
            Nuevo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_nuevoart)))
            Nuevo.click()
        ...
        try:
            Equivalencia = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_equivalencia))).text
            self.assertEqual("Equivalencia producto base", Equivalencia, "Campo visible")
            Log().info(" El campo 'Equivalencia producto base' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Equivalencia producto base' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            registro_tipodoc = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='COOL SHAVE']")))
            ...
            Log().error("No se encontró el registro de artículo, validar que la acción anterior haya finalizado,"
                        "que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            GuardarArticulo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_saveart)))
