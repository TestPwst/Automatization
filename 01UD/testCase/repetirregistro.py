...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("UBICACIONES EN DEPÓSITO : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Denvio = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_denvio))).text
            self.assertEqual("Detallar para envio", Denvio, "Campo visible")
            Log().info(" El campo 'Detallar envio' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Detallar envio' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Cdenvio = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_denvio)))
            Cdenvio.click()
