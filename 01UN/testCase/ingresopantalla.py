...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Unidades = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='unidades']")))
        ...
        try:
            ...
            self.assertEqual("UNIDADES DE ARTÍCULOS", Pantalla, "La pantalla ejecutada es correcta")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            Log().info(" La pantalla ejecutada es Unidades de Artículos.")
        ...
        try:
            ...
            Registrorepetido = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Códi']")))
            self.wait = WebDriverWait(self.driver, 60)
