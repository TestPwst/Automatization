...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            EventosContables = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='eventoscontables']")))
            action = ActionChains(self.driver)
            action \
                .click(EventosContables) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Eventos Contables")
        ...
        try:
            ...
            self.assertEqual("EVENTOS CONTABLES", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Eventos Contables.")
        ...
        try:
            ...
            if Registrorepetido.is_displayed():
                ...
                # Cambio Pestaña Parametros

                try:
                    Bparametros = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_parametros)))
                    Bparametros.click()
                    Log().info("Se hace el cambio a la pestaña Parametros para continuar con la eliminación del registro ")
                ...
                try:
                    ...
                    action \
                        ...
                        .pause(0) \
                        .click(Registroparametros) \
                        .pause(0) \
                        .release()
                    action.perform()
                    Log().info(" Se da clic en el registro de Parametros, para proceder a eliminarlo.")
                ...
                try:
                    Eliminaparametros = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_parametros)))
                    Eliminaparametros.click()
                    Log().info(" Se presiona el boton 'Eliminar', para eliminar el registro de Parametros.")
                ...
                try:
                    ...
                    Bitems.click()
                    Log().info("Se hace el cambio a la pestaña para continuar con la eliminación del registro ")
