...
class modificarregistro:
    ...
    def modificarregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            Curlftp.clear()
            Curlftp.send_keys(Configuracion.Murlftp)
        ...
        try:
            ...
            Cdirectorioftp.clear()
            Cdirectorioftp.send_keys(Configuracion.Mdirectorioftp)
        ...
        try:
            ...
            Cusuarioftp.clear()
            Cusuarioftp.send_keys(Configuracion.Musuarioftp)
        ...
        try:
            ...
            Cpasswordftp.clear()
            Cpasswordftp.send_keys(Configuracion.Mpasswordftp)
        ...
        try:
            ...
            Cmaxintentos.clear()
            Cmaxintentos.send_keys(Configuracion.Mmaxintentos)
            Log().info(" Ingresa el Maximo de Intentos del nuevo registro ")

        except Exception as e:
            Log().error(
                "No se pudo encontrar el campo para ingresar el Maximo de intentos, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
