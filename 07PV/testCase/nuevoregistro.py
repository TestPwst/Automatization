...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            Log().error("No se dió click en la opoción BackOffice y Mobile, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Nuevoporarticulo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_porarticulo)))
            Nuevoporarticulo.click()
            Log().info(" Se presiona el boton 'Nuevo', para crear otro nuevo registro de Por Articulo.")
        ...
        try:
            Guardarporarticulo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_porarticulo)))
            Guardarporarticulo.click()
            Log().info(" Se presiona el boton 'Guardar', para guardar el seguhdo registro de Por articulo.")
