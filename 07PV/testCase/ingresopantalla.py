        ...
        try:
            ...
            if Registrorepetido.is_displayed():
                ...
                try:
                    ...
                    Bporarticulo.click()
                    Log().info("Se hace el cambio a la pestaña Por Articulo para continuar con la eliminación de Por Articulo")
                ...
                try:
                    ...
                    Log().error("No se dipo click en el botón Eliminar, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                    self.driver.get_screenshot_as_file(img_name)
                ...
                try:
                    Eliminaporarticulo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_porarticulo)))
                    Eliminaporarticulo.click()
                    Log().info(" Se presiona el boton 'Eliminar', para eliminar el tercer registro de Por Articulo.")
