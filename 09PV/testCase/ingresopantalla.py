...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Log().error("La pantalla ejecutada no es correcta, validar que la acción anterior haya finalizado, que el nombre de la pestaña sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            if Registrorepetido.is_displayed():
                ...
                try:
                    self.wait = WebDriverWait(self.driver, 5)
                    Registroporarticulo = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='MARLBORO GOLD KS SOF 20 (FA01004)']")))
                    self.wait = WebDriverWait(self.driver, 60)
                ...
                try:
                    self.wait = WebDriverWait(self.driver, 5)
                    Registroporarticulo = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='MARLBORO MNT KS BOX 20 (FA01008)']")))
                    self.wait = WebDriverWait(self.driver, 60)
