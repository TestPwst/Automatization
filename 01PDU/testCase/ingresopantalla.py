...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Perfilesusuario = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='perfilesusuario']")))

            action = ActionChains(self.driver)
            action \
                .click(Perfilesusuario) \
                .pause(0) \
                .release()
            action.perform()

            Log().info(" Abre la pantalla de Perfiles de usuario")
        ...
        try:
            ...
            self.assertEqual("PERFILES DE USUARIOS", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Perfiles de Usuario.")
            time.sleep(1)
        ...
        try:
            ...
            Registrorepetido = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='"+Configuracion.Inomnegocio+"']")))
            self.wait = WebDriverWait(self.driver, 60)
