...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            cheques = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_che']")))

            action = ActionChains(self.driver)
            action \
                .move_to_element(cheques) \
                .pause(0) \
                .release()
            action.perform()

            chequeras = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='chequeras']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de chequeras")
        ...
        try:
            ...
            self.assertEqual("CHEQUERAS", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es chequeras")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='100']")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='101']")
            if Registrorepetido.is_displayed():
                try:
                    action = ActionChains(self.driver)
                    action \
