...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='1000']")))
        ...
        try:
            ...
            Log().info("Se hace el cambio a la pestaña Stock Mercaderias para continuar con la eliminación del registro")
        ...
        try:
            ...
            action \
                ...
                .pause(0) \
                .click(Registrostockmerca) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Se da clic en el registro de Stock Mercaderias, para proceder a eliminarlo.")
        ...
        try:
            Eliminastockmerca = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Elimina_stockmercaderia)))
            Eliminastockmerca.click()
            Log().info(" Se presiona el boton 'Eliminar', para eliminar el registro de Stock Mercaderias.")
        ...
        Registro4 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='1000']")))
        ...
        try:
            ...
            Log().error("No se sió click en el botón Aceptar, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Depositos.")
            time.sleep(2)
