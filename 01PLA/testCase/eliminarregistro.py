...
class eliminarregistro:
    ...
    def eliminarregistro(self, conditions, Configuracion):
        ...
        try:
            Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Testing1']")))
            time.sleep(1)
            Registro3 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Testing1']")))
        ...
        try:
            ...
            Cierra_todo.click()
            Log().info(" Se presiona el boton 'Cerrar', para cerrar la pantalla de Planometrias.")
            time.sleep(2)
