...
import ingresopantalla, nuevoregistro, repetirregistro, modificarresgistro, eliminarregistro
...
class Test(unittest.TestCase):
    """Escenarios de Categorías Fiscales"""
    ...
    def test_004(self):
        ...
        global continua
        if (continua):
            success = modificarresgistro.modificarregistro.modificarregistro(self, conditions, Configuracion)
            if (success == False):
                try:
                    Cierra_modal = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_cerrar_ventana)))
                    Cierra_modal.click()
