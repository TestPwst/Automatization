...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("CATEGORÍAS FISCALES : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Ccodigo_alt = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_codigo_alt)))
            Ccodigo_alt.send_keys(Configuracion.Icodigo_alt)
            Log().info(" Ingresa el codigo alternativo del nuevo registro igual al anterior")

        except Exception as e:
            Log().error("No se pudo ingresar el campo código alternativo, validar que la acción anterior haya finalizado,"
                        "que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
        ...
        try:
            ...
            Log().error("No se dio click al botón Cerrar de la ventana, validar que la acción anterior haya finalizado,"
                        "que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
