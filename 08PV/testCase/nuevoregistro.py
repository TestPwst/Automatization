...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            Brestxproducto = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_restxproducto)))
            Brestxproducto.click()
            Log().info("Se hace el cambio a la pestaña Restricción por producto para continuar con el registro nuevo")

        except Exception as e:
            Log().error("No se dió click en el botón Restricción por producto, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Nuevorestxproducto = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_restxproducto)))
            Nuevorestxproducto.click()
            Log().info(" Se presiona el boton 'Nuevo', para crear un nuevo registro de Restricción por Producto.")
        ...
        try:
            Tiporestriccion = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_tiporest))).text
            self.assertEqual("Tipo Restricción", Tiporestriccion, "Campo visible")
            Log().info(" El campo 'Tipo Restricción' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Tipo Restriccióno' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Ctiporest = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_tiporest)))
            Ctiporest.click()

            registro_tiporest = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Prohibido']")))

            action = ActionChains(self.driver)
            action \
                .click(registro_tiporest) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Se dió click en la opción Prohibido.")

        except Exception as e:
            Log().error("No se dió click en la opción Prohibido, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Guardarrestxproducto = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_restxproducto)))
            Guardarrestxproducto.click()
            Log().info(" Se presiona el boton 'Guardar', para guardar el registro de Restricción por producto.")
