...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("ZONAS DE REPARTO : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Repartidor = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_repartidor))).text
            self.assertEqual("Repartidor", Repartidor, "Campo visible")
            Log().info(" El campo 'Repartidor' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Repartidor' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Log().error(
                "El campo 'Unidad Negocio' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Crepartidor = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_repartidor)))
            Crepartidor.send_keys(Configuracion.Irepartidos)
            Log().info(" Ingresa el repartidor del nuevo registro ")

        except Exception as e:
            Log().error(
                "No se pudo encontrar el campo para ingresar el repartidor, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
