...
class PantallaZR:
    ...
    def PantallaZR(self):
        ...
        Zonas = self.driver.find_element(By.XPATH, "//*[@id='_tablas_zonas']")
        ZonasReparto = self.driver.find_element(By.XPATH, "//*[@id='zonasreparto']")
        ...
        try:
            ...
            time.sleep(3)
            Log().info(" Abre la pantalla de unidades")

        except Exception as e:
            Log().error(
                "No se pudo encontrar el menu indicado, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
        ...
        try:
            ...
            self.assertEqual("ZONAS DE REPARTO", Pantalla, "La pantalla ejecutada es correcta")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            Log().info(" La pantalla ejecutada es Zonas Bancarias.")
