...
class AgregarZR:
    ...
    def AgregarZR(self):
        ...
        try:
            Ccodigo = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_codigoZR)))
            Ccodigo.send_keys(Configuracion.IcodigoZR)
        ...
        try:
            Ccodigoalter = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_codigoaltZR)))
            Ccodigoalter.send_keys(Configuracion.IcodigoaltZR)
        ...
        try:
            Cdescripcion = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_descripcionZR)))
            Cdescripcion.send_keys(Configuracion.IdescripcionZR)
        ...
        try:
            ...
            Log().error("No se pudo encontrar el campo para ingresar el repartidor, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            CUnidadNegocio = self.wait.until(conditions.visibility((By.XPATH, Configuracion.ayuda_unidadnegocioZR)))
