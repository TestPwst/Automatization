...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            PoliticasVenta = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_pol']")))

            action = ActionChains(self.driver)
            action \
                .move_to_element(PoliticasVenta) \
                .pause(0) \
                .release()
            action.perform()

            CPoliticasVenta = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='clasificacionesPV']")))

            action = ActionChains(self.driver)
            action \
                .click(CPoliticasVenta) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Clasificaciones de Politicas de Venta")
        ...
        try:
            ...
            self.assertEqual("CLASIFICACIÓN DE POLÍTICAS DE VENTAS", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Clasificaciones de Políticas de Venta.")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='ClasifPV01']")
