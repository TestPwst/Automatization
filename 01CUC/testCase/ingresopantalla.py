...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Clasifasientos = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='clasificacionescc']")))

            action = ActionChains(self.driver)
            action \
                .click(Clasifasientos) \
                .pause(0) \
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Clasificaciones de Cuentas Contables")
        ...
        try:
            ...
            self.assertEqual("CLASIFICACIÓN DE CUENTAS CONTABLES", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Clasificaciones de Cuentas Contables.")
