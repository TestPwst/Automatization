...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("MOTIVOS DE NO TOMA DE INVENTARIO : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Existencia0 = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_existenciacero))).text
            self.assertEqual("Existencia cero", Existencia0, "Campo visible")
            Log().info(" El campo 'Existencia cero' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Existencia cero' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Cexistencia0 = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_existenciacero)))
            Cexistencia0.click()
            Log().info(" Hizo click en el checkbox ")
            
        except Exception as e:
            Log().error("No se pudo encontrar el campo envio, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
