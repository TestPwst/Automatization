...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("TIPOS DE IDENTIFICACIÓN : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        try:
            Habilitado = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_habilitado))).text
            self.assertEqual("Habilitado", Habilitado, "Campo visible")
            Log().info(" El campo 'Habilitado' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Habilitado' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Chabilitado = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_habilitado)))
            Chabilitado.click()
            Log().info(" Hizo click en el checkbox Habilitado ")
            time.sleep(1)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            ...
            Log().error("No se dió click en el checkbox habilitado, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
