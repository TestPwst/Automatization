...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            Grupoadhoc = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_grupoadhoc))).text
            self.assertEqual("Grupo Ad-Hoc", Grupoadhoc, "Campo visible")
            Log().info(" El campo 'Grupo Ad-Hoc' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Grupo Ad-Hoc' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Cgrupoadgoc.click()
            registro_grupoadhoc = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Grupo Política Prueba 1']")))
        ...
        try:
            Articuloabonificar = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_articuloabonificar))).text
            self.assertEqual("Artículo a Bonificar", Articuloabonificar, "Campo visible")
            Log().info(" El campo 'Artículo a Bonificar' si se encuentra visible.")

        except Exception as e:
            Log().error("El campo 'Artículo a Bonificar' no se muestra visible, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Carticuloabonificar.send_keys(Configuracion.Iarticuloabonif)
            Log().info(" Ingresa el Articulo a bonificar del nuevo registro ")

        except Exception as e:
            Log().error("No se encontró el campo para Articulo a Bonificar, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
