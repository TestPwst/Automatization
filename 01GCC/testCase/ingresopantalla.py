...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Gruposcc = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='gruposcentrocosto']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Grupos Centro de Costo")
        ...
        try:
            ...
            self.assertEqual("GRUPOS DE CENTRO DE COSTO", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Grupos Centro de Costo")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='Codigogcc01']")
            if Registrorepetido.is_displayed():
                ...
                # Cambio de pestaña y elimina registro
                ...
                try:
                    ...
                    Log().error(
                        "No se encontró el botón centro costo, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
                    time.sleep(2)
                    timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
                    img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
                    Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
                ...
                Registro4 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='prueba']")))
                ...
                Registro5 = self.wait.until(conditions.visibility((By.XPATH, "//span[text()='Codigogcc01']")))
