...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("GRUPOS DE CENTRO DE COSTO : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        # Cambio de pestaña centro costo y agregar nuevo
        ...
        try:
            ...
            Log().error("No se encontró el botón centro costo, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            time.sleep(2)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Guardarcentrocosto = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Guarda_centrocosto)))
            Guardarcentrocosto.click()
