...
class repetirregistro:
    ...
    def repetirregistro(self, conditions, Configuracion):
        ...
        try:
            Procevento = self.wait.until(conditions.visibility((By.XPATH, Configuracion.etiqueta_procevento))).text
            self.assertEqual("Procedimiento Evento", Procevento, "Campo visible")
            Log().info(" El campo 'Procedimiento Evento' si se encuentra visible.")

        except Exception as e:
            Log().error(
                "El campo 'Procedimiento Evento' no se muestra visible, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            registro_tipoaccion = self.wait.until(conditions.visibility((By.XPATH, "//li[text()='Ejecutar Proc. de Evento']")))
        ...
        try:
            ...
            time.sleep(1)
            Cprocevento = self.wait.until(conditions.visibility((By.XPATH, Configuracion.campo_procevento)))
            Cprocevento.send_keys(Configuracion.Iprocevento)
            Log().info(" Ingresa el Procedimiento Evento del nuevo registro ")

        except Exception as e:
            Log().error("No se pudo encontrar el campo para ingresar el Procedimiento Evento, revise si el xpath sigue siendo el mismo, para mas detalles del error consulte el reporte")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
