...
class nuevoregistro:
    ...
    def nuevoregistro(self, conditions, Configuracion):
        ...
        try:
            ...
            self.assertEqual("PRORRATEOS POR CENTROS DE COSTO : PROPIEDADES", Crea, "La pantalla ejecutada es correcta")
        ...
        # Cambio de pestaña centro de costos
        ...
        try:
            ...
            Log().error("No se dió click al botón centro de costo, revise si el xpath sigue siendo el mismo, revise la accion anterior si el xpath es el correcto o el tiempo de la accion")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            Nuevocentrocosto = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_Nuevo_centrocosto)))
            Nuevocentrocosto.click()
        ...
        # validacion de etiquetas pestaña centro costo
        try:
            ...
            Log().error(
                "El campo 'Centro Costo' no se muestra visible, revise si el xpath sigue siendo el mismo, revise la accion anterior si el xpath es el correcto o el tiempo de la accion")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            self.assertEqual("Porcentaje", porcentaje, "Campo visible")
            Log().info(" El campo 'Porcentaje' si se encuentra visible.")
            
        except Exception as e:
            Log().error(
                "El campo 'Porcentaje' no se muestra visible, revise si el xpath sigue siendo el mismo, revise la accion anterior si el xpath es el correcto o el tiempo de la accion")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            ...
            Log().error("No se encontró el registro de categoria, revise si el xpath sigue siendo el mismo, revise la accion anterior si el xpath es el correcto o el tiempo de la accion")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
        ...
        try:
            btnguardar = self.wait.until(conditions.visibility((By.XPATH, Configuracion.btn_guardarcosto)))
            btnguardar.click()
            Log().info(" Se presiona el boton 'guardar', para guardar el registro.")

        except Exception as e:
            Log().error(
                "No se dió click al botón Eliminar, revise si el xpath sigue siendo el mismo, revise la accion anterior si el xpath es el correcto o el tiempo de la accion")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
