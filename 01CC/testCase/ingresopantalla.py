...
class ingresopantalla:
    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            Centrocostos = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='centrocosto']")))
            ...
            action \
                ...
                .release()
            action.perform()
            Log().info(" Abre la pantalla de Centros Costo")
        ...
        try:
            ...
            self.assertEqual("CENTROS COSTO", Pantalla, "La pantalla ejecutada es correcta")
            Log().info(" La pantalla ejecutada es Centros Costo.")
            ...
            Log().error("La pantalla ejecutada no es correcta, validar que la acción anterior haya finalizado, que el nombre la pantalla sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='3']")
        ...
        try:
            ...
            Log().error("No se dió click a el botón Nuevo, validar que la acción anterior haya finalizado, que el xpath sea el correcto o que la página no presente lentitud")
            time.sleep(3)
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
